...
<div class="modal fade" id="addCustomer" data-bs-keyboard="false" tabindex="-1" aria-labelledby="addCustomerLabel"
    ...
    <div class="modal-dialog  modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addOfflineCustomer11" action="{{ route('franchise-add-offline-customerCO') }}" method="POST">
                    @csrf
                    <div class="row">
                        <div class="col-md-4">
...
<script type="text/javascript">
    ...
    $(function() {
        var table = $('.data-table').DataTable({
            ...
            ajax: "{{ route('franchise-partner-customer-orders') }}",
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data) {
                        ...
                        switch (data) {
                            case 0:
                                return "<span class='badge bg-danger me-1 text-white'>Not-Approve</span>"
                                break;

                            case 1:
                                return "<span class='badge bg-success me-1 text-white'>Approve</span>"
                                break;
                        }
    ...
    function GetCustomerTasks123(cust_id, orderId, service_id, is_approve) {
        console.log(cust_id);
            ...
            success: function(data) {
                ...
                $.each(data, function(index, item) {
                    ...
                    $.each(item, function(index, item1) {
                        ...
                        if (item1.emi_unpaid_amount > 0) {
                            remark += `
                                <hr>
                                <div class="d-flex justify-content-between">
                                    <form id="updateEMI" action="{{ route('fe-update-emi') }}" method="post"  enctype="multipart/form-data">
                                        <div class="row">
                                            @csrf
                                            <div class="col-md-6">
    ...
    // add work effort
    function AddWorkEffort(cust_id, service_id, is_approve) {
        console.log('fdsgfdgdf');
    ...
    // customer tasks
    function GetCustomerTasks(cust_id, service_id, is_approve) {
        console.log(cust_id);
        $.ajax({
            type: "GET",
            url: "{{ route('Get-Customer-Tasks') }}",
                    ...
                    $.each(data, function(index, item) {
                        ...
                        $.each(item, function(index, item1) {
                            ...
                            if (item1.chat_from != cust_id) {
                                var chat_from = item1.from_name ? item1.from_name :
                                    item1.from_cust_name;
                                remark += `
                            <div class="direct-chat-msg right">
                        <div class="direct-chat-info clearfix">
                            <span class="direct-chat-name pull-right">${chat_from}</span>
                        ...
                        <div class="direct-chat-text">
                            ...
                            `;
                                ...
                                var chat_to = item1.from_name ? item1.from_name :
                                    item1.from_cust_name;
                                remark += `<div class="direct-chat-msg">
                        <div class="direct-chat-info clearfix">
                            <span class="direct-chat-name pull-left">${chat_to}</span>
    ...
    function GetAllCustomers() {
        ...
        var table = $('.data-table').DataTable({
            ...
            ajax: {
                url: "{{ route('franchise-customer-filter') }}",
                data: {
                    category_id: category_id,

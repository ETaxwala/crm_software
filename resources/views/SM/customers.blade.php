    ...
    <div class="modal-dialog  modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addOfflineCustomer" action="{{ route('add-offline-customer') }}" method="POST">
                    @csrf
                    <div class="row">
                        <div class="col-md-4">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-control" onchange="GetServicePrice(this.value)" name="customer_service"
                                id="serviceSelect" required>
...
<div class="modal fade" id="PaymentBox" data-bs-keyboard="false" tabindex="-1" aria-labelledby="PaymentBoxLabel"
    aria-hidden="true">
    <div class=" modal-dialog-scrollable modal-dialog modal-md" role="document">
        <div class="modal-content">
            <div class="modal-body">
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <div class="modal-header bg-danger">
                <h5 class="modal-title text-white">Assign Task</h5>
            </div>
            <div class="modal-body">
                <div class="modal-body">
                    ...
                    <form id="AssignCutomerTaskTo" class="row g-3" action="{{ route('assign-cust-task-to') }}"
                        ...
                        <div class="container">
                            <div class="input-group mb-3">
                                ...
                                {{-- <span class="input-group-text bg-danger text-white">Assign</span> --}}
                                <button type="submit" class="input-group-text bg-danger text-white">Assign</button>
                            </div>
...
<script type="text/javascript">
    ...
    $(function() {
        var table = $('.data-table').DataTable({
            ...
            ajax: "{{ route('sales-manage-customers') }}",
    ...
    function showPaymentBox(cust_id) {
        $.ajax({
            type: "GET",
            url: "{{ route('Get-Customer-Payment-Details') }}",
            data: {
                cust_id: cust_id,
            },
            success: function(data) {
                ...
                $.each(data, function(index, item) {
                    let ss =
                        `Payment Screenshot <br><img class="payment__screenshot" src="/payment/${item.payment_screenshot}" alt="${item.payment_screenshot}" width="500" height="100">`;

                    let screenshot = item.payment_screenshot ? ss : 'No screenshot provided';
                    ...
                    remark += `
                                <div>
                                    ...
                                    <br><br>
                                    <div class="row">
                                        <div class="col-md-4">Total Amount - ${item.emi_total_amount}</div>
                                        <div class="col-md-4">Paid Amount - ${item.emi_paid_amount}</div>
                                        <div class="col-md-4">Unpaid Amount - ${item.emi_unpaid_amount}</div>
                                    </div>
                                    <form id="approveCustomer" action="{{ route('approve-customer') }}" method="post">
                                        @csrf
                                        <div class="row">
                                            <input type="hidden" name="customer_id" value="${item.customer_id}">
                                            <div class="col-md-12"><button class="btn btn-warning" type="submit">Approve Customer</button></div>
                                        </div>
                                    </form>
                ...
                $.each(data, function(index, item) {
                    ...
                    $.each(item, function(index, item1) {
                        ...
                        const due_date = item1.emi_due_date ? item1.emi_due_date :
                            'Not required';
                        const unpaid_amount = item1.emi_unpaid_amount ? 'RS ' + item1
                            .emi_unpaid_amount : 'No Pending Payment';
                        ...
                        remark += `
                            <div class="container">
                                ...
                                <div class="d-flex justify-content-between">
                                    ...
                                    <span><b>Next Due Date:</b> <br/>  ${due_date}</span>
                                </div>
                            </div>
                            `;
    ...
    // approve payment
    $(document).ready(function() {
        $('#approveCustomer').on('submit', function(e) {
            e.preventDefault();
            ...
            $.ajax({
                ...
                success: function(response) {
                    $('.data-table').DataTable().ajax.reload(null, false);
                    toastr.success('Customer Approved!!');
                },
                error: function(error) {
                    toastr.error('Customer Not Approved!!');
                }
            });
        });
    });

...
<a href="/file-download/{{ $item->chat_id }}" download="{{$item->chat_file}}">Download {{$item->chat_file_name}}</a>
                ...
                <div class="ms-panel ms-panel-fh ms-widget ms-chat-conversations">
                    ...
                    <div class="ms-panel-body ms-scrollable ps ps--active-y">
                        ...
                        @foreach ($customers as $customer)
                            <div onclick="GetChatMsg({{ $customer->id }}, '{{ $customer->name }}')"
                            ...
                            <div class="ms-chat-textbox">
                                <ul class="ms-list-flex mb-0">
                                    ...
                                    <li class="ms-chat-text-controls ms-list-flex">
                                        ...
                                        <form id="sendDocs" action="{{ route('customer-chatting-from-operation2') }}" method="post"
    ...
    $(document).ready(function() {
        $('#sendDocs').on('submit', function(e) {
            ...
            $.ajax({
                ...
                contentType: false,
                success: function(response) {
                    var custID = document.getElementById('chat_cust_id2').value;
                    GetChatMsg(custID)
                    // $("#LeadDetails").modal('hide')
                    document.getElementById('sendDocs').reset();
                    // Show a success toast
                },
                error: function(error) {
                }

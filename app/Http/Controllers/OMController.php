...
class OMController extends Controller
{
    public function Dashboard(Request $request)
    {
        ...
        // dd($data);
        return view('OM.dashboard', compact('chattings', 'services'));
    }
    ...
    public function ManageEmployee(Request $request)
    {
        ...
        // dd($data);
        if ($request->ajax()) {
            $data = DB::select("SELECT * FROM users WHERE user_type = 7 AND added_by = $user_id ORDER BY id DESC");
            ...
            return Datatables::of($data)
                ->addColumn('action', function ($data) {
                  ...
                  <svg onclick="DeleteOperationEmployee(' . $data->id . ')" style="cursor: pointer" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
        ...
        return view('OM.manage_employee');
    }
    ...
    function GetAllTasks(Request $request)
    {
        ...
        <span>
                    <svg style="cursor:pointer" onclick="DeleteTask(' . $data->task_id . ')" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-trash-fill" viewBox="0 0 16 16">
                    <path d="M2.5 1a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1H3v9a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V4h.5a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H10a1 1 0 0 0-1-1H7a1 1 0 0 0-1 1zm3 4a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 .5-.5M8 5a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7A.5.5 0 0 1 8 5m3 .5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 1 0"/>
                    </svg>
                </span>';
            })
    ...
    {
        $data = DB::select("SELECT * FROM coupons WHERE is_active = 0 ORDER BY coupon_id DESC");
    ...
    function deleteTask(Request $request)
    {
        ...
        // DB::transaction(function () use ($task_id) {
            // Check and delete from operation_tasks
            if (DB::table('operation_tasks')->where('task_id', $task_id)->exists()) {
                DB::table('operation_tasks')->where('task_id', $task_id)->delete();
            }

            // Check and delete from task_assign
            if (DB::table('task_assign')->where('task_id', $task_id)->exists()) {
                DB::table('task_assign')->where('task_id', $task_id)->delete();
            }

            // Check and delete from customer_work_timeline
            if (DB::table('customer_work_timeline')->where('task_id', $task_id)->exists()) {
                DB::table('customer_work_timeline')->where('task_id', $task_id)->delete();
            }
        // });
    ...
    function deleteCustomer(Request $request)
    {
        ...
        $customer = DB::table('customers')->where('id', $customer_id)->get();

        DB::table('customers')->where('id', $customer_id)->delete();
        DB::table('orders')->where('$customer_id', $customer_id)->delete();

        DB::table('customer_chatting')->where('service_id', $service_id)->where('chat_from', $customer_id)->where('chat_to', $customer_id)->delete();
        DB::table('customer_documents')->where('service_id', $service_id)->where('cd_from', $customer_id)->where('cd_to', $customer_id)->delete();

        DB::table('customer_emi')->where('customer_id', $customer_id)->delete();
        DB::table('customer_dues')->where('customer_id', $customer_id)->delete();


        DB::table('customer_work_timeline')->where('cust_id', $customer_id)->delete();

        // DB::table('leads')->where('id', $customer[0]->lead_id)->delete();

        DB::table('operation_tasks as ot')->where('ot.cust_id', $customer_id)
        ->leftJoin('task_assign as ta', 'ta.task_id', '=', 'ot.task_id')->delete();
